...
export default function Header() {
  ...
  // Deriving real total item values dynamically
  const cartCount = getCartCount();
  const wishlistCount = wishlist ? wishlist.length : 0;

  const isOnResultsPage = location.pathname === "/shop/all";

  // Dynamic token session verifier
  const isAuthenticated = (): boolean => {
    const token = localStorage.getItem("token");
    return !!token && !!user;
  };

  // Secure navigation guard for wishlist links
  const handleWishlistClick = (e: React.MouseEvent) => {
    e.preventDefault();
    if (!isAuthenticated()) {
      alert("✨ Please sign in to access your personal wishlist!");
      setMenuOpen(false); // Close mobile menu drawer if open
      navigate("/login");
    } else {
      setMenuOpen(false);
      navigate("/wishlist");
  ...
  return (
    <>
      ...
      >
        <div className="eloura-inner">
          ...
          <div className="eloura-actions">
            ...
            {/* PROTECTED DESKTOP WISHLIST ICON */}
            <button 
      ...
      >
        ...
        {/* Mobile Icon Row Badges */}
        <div className="mobile-icons-row">
          {/* PROTECTED MOBILE WISHLIST BUTTON */}
          <button 
            onClick={handleWishlistClick} 
            className="icon-btn" 
            title="Wishlist"
      ...
      {open && (
        <div className="user-tooltip">
          ...
          <button 
            ...
            onClick={(e) => { onWishlistClick(e); setOpen(false); }}
          >
            Wishlist
          </button>
          <button
