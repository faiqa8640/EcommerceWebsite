...
// used to post the image 
export const handleImageUpload = async (req: Request, res: Response) => {
  try {
    if (!req.file) { // file comes from the multer .. tht if the file exist
    ...
    const folder = (req.body?.folder as string) || "products"; // decide the folder based on send by the frontend
